...
function reverseString(str) {
    ...
    let sliceSent = str.slice(0, -1);
    return str[lastIdx].concat(reverseString(sliceSent));
}
...
function pow(base, exponent) {
    ...
    if (exponent > 0) {
        ...
        return base * ( 1 / pow(base, (exponent * -1) + 1));
    }
}
...
function flatten(data) {
    ...
    data.forEach((ele) => {
        if (Array.isArray(ele)) {
            flattenArr = flattenArr.concat(flatten(ele));
        } else {
            flattenArr.push(ele)
        }
    })
...
function fileFinder(directories, targetFile) {
    for(let folder in directories) {
        if (folder === targetFile || fileFinder(directories[folder], targetFile)) {
            return true;
        }
    }
    return false;
...
function pathFinder(directories, targetFile) {
    for(let folder in directories) {
        ...
        let res = pathFinder(directories[folder], targetFile);
        if (res !== null) {
            return folder + res;
        }
    }
