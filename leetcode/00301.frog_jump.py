...
# the water. Given a list of stones positions (in units) in sorted ascending order, determine 
# if the frog can cross the river by landing on the last stone. Initially, the frog is on the 
# first stone and assumes the first jump must be 1 unit. If the frog's last jump was k units, 
# its next jump must be either k - 1, k, or k + 1 units. The frog can only jump in the forward direction.
...
class Solution:
    def canCross(self, stones: List[int]) -> bool:
        n = len(stones)
        memo = {}
        return self.dfs(0, stones, n, 0, memo)

    def dfs(self, i, stones, n, k, memo) -> bool:
        if i >= n - 1: return True
        if (i, k) in memo: return memo[(i, k)]
        
        for j in range(i + 1, n):
            diff = stones[j] - stones[i]
            if diff == k + 1 or diff == k - 1 or diff == k:
                can_cross_river = self.dfs(j, stones, n, diff, memo)
                if can_cross_river:
                    memo[(i, k)] = True
                    return True
