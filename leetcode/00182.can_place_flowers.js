// 605. Can Place Flowers

// You have a long flowerbed in which some of the plots are planted, and some are 
// not. However, flowers cannot be planted in adjacent plots.

// Given an integer array flowerbed containing 0's and 1's, where 0 means empty 
// and 1 means not empty, and an integer n, return if n new flowers can be 
// planted in the flowerbed without violating the no-adjacent-flowers rule.

// Example 1:
// Input: flowerbed = [1,0,0,0,1], n = 1
// Output: true

// Example 2:
// Input: flowerbed = [1,0,0,0,1], n = 2
// Output: false

var canPlaceFlowers = function(flowerbed, n) {
    for(let i = 0; i < flowerbed.length; i++) {
        let currentFlower = flowerbed[i];
        let prevFlower = flowerbed[i - 1] === undefined ? 0 : flowerbed[i - 1];
        let nextFlower = flowerbed[i + 1] === undefined ? 0 : flowerbed[i + 1];
        
        if(currentFlower === 0 && prevFlower === 0 && nextFlower === 0) {
            if(n > 0) {
                flowerbed[i] = 1;
                n--;
            } else {
                return true;
            }
        }
    }
    return n === 0 ? true : false;
};

// time o(n) | space o(1)
// https://leetcode.com/problems/can-place-flowers/