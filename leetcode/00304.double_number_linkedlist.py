...
# leading zeroes. Return the head of the linked list after doubling it.
...
#     0 <= Node.val <= 9
#     The input is generated such that the list represents a number that does not have leading 
...
class Solution:
    def doubleIt(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        while current:
            numbers.append(str(current.val))
            current = current.next
        
        doubled = str(int("".join(numbers)) * 2)
