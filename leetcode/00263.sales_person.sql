...
-- Each row of this table contains information about one order. This includes the ID of the 
-- company, the ID of the salesperson, the date of the order, and the amount paid.
...
with 
    red_company as (select com_id from Company where name = 'RED'),
    red_salesmen as (
        select sales_id
        from Orders
        where com_id = (select com_id from red_company)
    )

select name
from SalesPerson
where sales_id not in (select sales_id from red_salesmen)