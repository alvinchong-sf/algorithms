...
Time: constructor, birth, death | O(1)
Time: getInheritanceOrder | O(n)
Space: Overall | O(n)
"""
...
class ThroneInheritance:
    ...
    def getInheritanceOrder(self) -> List[str]:
        order = []
        self.dfs(self.king, self.kingdom, order)
        return order

    def dfs(self, parent, kingdom, order):
        if parent not in self.death_list:
            order.append(parent)
        for child in kingdom[parent]:
            self.dfs(child, kingdom, order)