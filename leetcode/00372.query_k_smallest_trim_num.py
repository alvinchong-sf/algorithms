...
of only digits. You are also given a 0-indexed 2D integer array queries where queries[i] = 
...
Note:
    ...
    digits remain. Strings in nums may contain leading zeros.
    ...
    def smallestTrimmedNumbers(self, nums: List[str], queries: List[List[int]]) -> List[int]:
        ...
        for k, trim in queries:
            ...
            for idx, num in enumerate(nums):
                digits = deque([d for d in num])
                while len(digits) != trim:
                    digits.popleft()
                int_digit = int("".join(digits))
                sorted_arr.append((int_digit, idx))
            sorted_arr.sort(key=lambda x: (x[0], x[1]))
            _, result_idx = sorted_arr[k - 1]
