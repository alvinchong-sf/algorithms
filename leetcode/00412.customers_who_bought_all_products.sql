...
in the Product table. Return the result table in any order. The result format is in the following example.
...
where customer_id not in (
    select customer_id
    from (
        select distinct customer_id, Product.product_key
        from Customer
        cross join Product
    )
