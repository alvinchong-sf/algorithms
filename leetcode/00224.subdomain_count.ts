...
function subdomainVisits(cpdomains: string[]): string[] {
    ...
    for (const cpd of cpdomains) {
        const [strCount, domain] = cpd.split(" ");
        const count = parseInt(strCount);
        helper(count, domain, hashMap);
    }
...
function helper(count: number, domain: string, hashMap: HashMap): void {
    ...
    subdomains.shift();
    const newDomain = subdomains.join(".");
    helper(count, newDomain, hashMap);
}