...
nums. Return the array nums sorted in non-decreasing order based on the mapped values of its elements.
...
Time: O(nlog(n) + n * d) | Space: O(n)
"""
class Solution:
    def sortJumbled(self, mapping: List[int], nums: List[int]) -> List[int]:
        n = len(nums)
        hmap, mapped = {}, []
        for idx, map_num in enumerate(mapping):
            hmap[idx] = map_num
        for num in nums:
            s = str(num)
            temp = []
            for digit in s:
                temp.append(str(hmap[int(digit)]))
            mapped.append(int("".join(temp)))
        sorted_arr = []
        for i in range(n):
            sorted_arr.append((mapped[i], i, nums[i]))
        sorted_arr.sort(key=lambda x: (x[0], x[1]))
        return [res for _, _, res in sorted_arr]