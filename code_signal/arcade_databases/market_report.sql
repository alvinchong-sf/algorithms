-- Your company is planning to expand internationally very soon. You have been tasked with 
-- preparing a report on foreign markets and potential competitors.
...
-- and competitors. The first column should contain the country name, and the second column 
-- should contain the number of competitors in this country. The table should be sorted by 
-- the country names in ascending order. In addition, it should have an extra row at the 
-- bottom with the summary, as described above.
...
BEGIN
    ...
    from (
        ...
        union all
        (
            select 'Total:', count(*) as competitors
            from foreignCompetitors
        )
    ) as competitions
    order by
        case
            when country = 'Total:' then 1000000
        end,
        country
        ;
END