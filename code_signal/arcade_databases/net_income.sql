-- You own a small company, and you keep track of its income in the accounting table, which has 
...
-- Given the accounting table, write a select statement which returns three columns: year, 
-- quarter and net_profit. The first column should contain the year, the second one should 
-- contain the quarter of that year, and the third one should contain the net income 
-- (profit - loss difference) of your company during that period. The output should be sorted by 
-- the year in ascending order. If there are several rows with the same year, sort them by the 
-- quarter in ascending order.
