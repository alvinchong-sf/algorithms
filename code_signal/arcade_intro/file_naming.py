...
# (assume 0-based indexing). Because all files must have unique names, files 
# created later with the same name as a file created earlier should have an 
...
def solution(names):
    ...
    for name in names:
        if name in hash_map:
            val = hash_map[name]
            new_name = name + f"({val})"
            while name + f"({val})" in hash_map:
                val += 1
                new_name = name + f"({val})"
            
            result.append(new_name)
            hash_map[name] += 1
            if new_name in hash_map:
                hash_map[new_name] += 1
            else:
                hash_map[new_name] = val
        else:
            hash_map[name] = 1
            result.append(name)
