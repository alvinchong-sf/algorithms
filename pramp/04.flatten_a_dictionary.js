...
// string etc) or other dictionaries (dictionaries can be nested). However, 
...
// If you’re using a compiled language such Java, C++, C#, Swift and Go, you 
// may want to use a Map/Dictionary/Hash Table that maps strings (keys) to a 
// generic type (e.g. Object in Java, AnyObject in Swift etc.) to allow nested dictionaries.
...
//             "Key2.c.e" : "1"
//         }
// Important: when you concatenate keys, make sure to add the dot character between 
// them. For instance concatenating Key2, c and d the result key would be Key2.c.d.
...
const recHelper = (k, v, resultHash) => {
  for (const key in v) {
    let newKey;
     if (key === "") {
      newKey = k
    ...
    if (typeof(v[key]) === "object") {
      recHelper(newKey, v[key], resultHash);
    } else {
      resultHash[newKey] = v[key];
    }
  }
};