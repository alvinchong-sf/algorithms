...
// many hacking attacks and drug trafficking, has recently become a target for 
// the FBI. After intercepting some of their messages, which looked like complete 
...
function decrypt(word) {
  ...
  for (let i = 0; i < word.length; i++) {
    const currAscii = word.charCodeAt(i);
    let resultAscii = currAscii - accumulation;
    
    while (resultAscii < 97) {
      resultAscii += 26;
    }
    
    accumulation += resultAscii;
    
    const newChar = String.fromCharCode(resultAscii);
    decryptArr.push(newChar);
  }
